...
%%%%%%%%%%Portada %%%%%%%%%%%%%%%%%%%%%%%%%
\portadatesis{Adecuación Automatizada de Modelo de Elevación Digital para usos hidrológicos}{Cristian Guerrero Córdova}{UNIVERSIDAD NACIONAL DEL CÓRDOBA}{Marzo, 2019}{2019}{Sergio Masuelli}{CONAE, Córdoba, Argentina}
...
Como trabajo de Tesis de Licenciatura en Ciencias de la Computación desarrollé un Simulador Hidrológico para llanuras (Guerrero Cordova 2013) basado en trabajo desarrollado por Vazquez \textit{et al.} (2007).

Un dato importante de entrada en forma de grilla digitalizada es el DEM. Para usos hidrológicos el DEM debe ser acondicionado apropiadamente para representar el flujo del agua, como se detalla en Masuelli \textit{et al.} (2009). En dicho trabajo se realizaron distintos procesos (transformada de Fourier, aplicación de filtros, etc.) de forma manual aplicados a la zona de llanura de la Provincia de Buenos Aires. Para poder realizar simulaciones hidrológicas en distintas áreas, es importante tener a disposición una herramienta que realice el preprocesamiento del DEM de manera automatizada para que el DEM sea apto para su posterior uso hidrológico, es por eso que se consideró importante desarrollar esta herramienta. De esta manera el uso del Simulador Hidrológico puede hacerse extensivo a diferentes áreas de interés.

\section{Objetivo}


Desarrollar una herramienta que implemente la cadena de procesamientos automatizada necesaria para obtener como resultado un DEM que esté adecuado para usos hidrológicos. De esta manera se podrá adecuar al DEM global de SRTM para usos hidrológicos sobre cualquier llanura. Para ello se realizaron las siguientes tareas:

\begin{itemize}	
	\item Relevar los modelos de elevación globales, existentes de manera gratuita y libres, para así determinar el más adecuado para tomar como base del procesamiento. Para el trabajo de base se utilizó el DEM de la misión SRTM.
...
\item Cuando la intensidad de precipitación es menor que la capacidad de infiltración y el contenido de humedad del suelo o roca es mayor o igual a su capacidad de campo: Como el suelo o roca se encuentra en una condición cercana a la capacidad de campo, parte de la precipitación se convertirá eventualmente en escurrimiento sobre el terreno; sin embargo, los volúmenes seguirán siendo de poca cuantía. En este caso el flujo subsuperficial será importante debido a que ocurren intercambios de agua de manera horizontal y vertical.
...
\item Cuando la intensidad de precipitación es mayor que la capacidad de infiltración y el contenido de humedad del suelo o roca es mayor o igual a su capacidad de campo: En este caso, al encontrarse el suelo o roca en una condición cercana a la saturación, no permitirá una infiltración importante, de modo que la mayor parte se convertirá en escurrimiento sobre el terreno. El flujo subsuperficial también será importante debido a que ocurren intercambios de agua de manera horizontal y vertical. Cuando la parte somera de un suelo no permite una infiltración importante, se forma el denominado flujo Hortoniano, es decir, la saturación en un suelo o roca tendrá lugar sólo en una porción cercana a la superficie, siendo incapaz el frente de humedad  de avanzar a mayor profundidad, favoreciendo de esta manera al escurrimiento sobre el terreno.
\end{itemize}
...
Todos los valores de elevación son enteros binarios con representación de signo y magnitud, correctamente justificados, de 2 bytes. El bit de signo es el más significativo. Los valores negativos no son complementados. Estos valores permiten un rango de entre ± 32767 metros, sin embargo en la práctica, la elevación del terreno no deben exceder los +9000 metros o los -12000 metros. Los datos nulos son representados por valores con todos los bits seteados en 1, es decir -32767. Aunque Jarvis \textit{et al.} (2008) hicieron desarrollos para corregir los valores sin datos, aun existen estos píxeles erróneos dentro de la imagen.
...
Después de realizar una serie de pruebas locales, se decidió aplicar una combinación de datos SRTM-3 y DTED-1. Para cada píxel se usó el valor mínimo de cualquiera de los DEM, ya sea en SRTM-3 o en DTED-1 para generar un modelo de elevación inicial HydroSHEDS. Se conserva la menor de las dos alturas en los datos de elevación combinados, los cual se considera deseable para la identificación posterior de direcciones de drenaje, ya que para uso hidrológico el agua fluirá de mayores alturas hacia menores alturas.
...
Se identificaron de modo manual áreas dedicadas a cultivos, esto se puede lograr fácilmente observando en detalle las imágens de alta resolución de Google Earth (ver figura \ref{DatosDeVerdadCultivos}). Cabe aclarar que la identificación de estas áreas de cultivos se realizó con el objetivo de determinar elementos en la imagen que no son arboledas y de esta manera separar mejor las clases en la clasificación. Estos polígonos fueron creados en formato de archivo shape.
...
Este dato de entrada fue descripto de manera detallada en la sección \ref{SRTMDefincion}; puede descargarse de manera gratuita del sitio WEB del Servicio Geológico de Estados Unidos (USGS) del siguiente enlace: \\ \hyperref[http://srtm.csi.cgiar.org/srtmdata/]{``http://srtm.csi.cgiar.org/srtmdata/"}.
...
Para esto se realizan recortes de estos dos primeros cuartos (figuras \ref{firstQuarter} y \ref{secondQuarter}) dejando fuera también los valores que corresponden con las líneas centrales (que poseen altos valores también) por lo que se adopta un margen de 10 píxeles para excluir las líneas centrales.
...
\textbf{Armado de la imagen con los 4 cuartos:} Una vez obtenidos los 4 cuartos, deben ser unidos en una imagen grande, del tamaño de la original. En este caso se debe recordar si tanto filas como columnas eran de cantidad impar. Ya que en ese caso, al armar la imagen de máscara completa se debió completar tal fila o columna faltante al momento de dividir la imagen por la mitad. Se obtiene la imagen que se puede ver en la figura \ref{maskFourierComplete}
...
Para poder aplicar la transformada inversa se debe retornar las frecuencias cero a su posición inicial.
...
Para detección de las cavidades mencionadas eso se definió un filtro de Mayoría con una ventana de 11 x 11, el cual opera de la siguiente manera: si el píxel central pertenece a una laguna si la mayoría de sus vecinos dentro de la ventana de 11 x 11 tienen el mismo valor. En caso de ser así, el píxel del núcleo conserva su valor original, (el que trae del DEM HydroSHEDS), caso contrario, toma el valor 0. En la figura \ref{filtroMayoria} se puede observar un zoom a una parte del área de estudio en donde se observan las lagunas detectadas:
...
Luego de la máscara obtenida en el paso anterior, se observó la presencia de numerosos píxeles aislados, o agrupados solo en conjuntos de dos o tres píxeles. Estos valores son no deseados, ya que no representan algún cuerpo de agua, son sólo puntos aislados, es por eso que no deben formar parte de las lagunas detectafsd. Para la eliminación de estos casos se procedió a aplicar de manera secuencial:
...
\end{figure}
		...
		\textbf{Expansión:} A la máscara resultado de la aplicación del filtro de erosión se le aplica un filtro de expansión. De esta manera serán expandidas las lagunas detectadas por el filtro de mayoría (y posteriormente erosionadas) pero que no desaparecieron con el filtro de erosión, de esta manera se consigue eliminar de la máscara píxeles con valores 1 aislados ya que al estar ausentes (luego del filtro de erosión), con la aplicación del filtro de expansión no producen nada para expandir. Esto se puede observar en la figura \ref{expansionZoom}:	
...
\begin{figure}[H]
	...
	\subfigure[Resultado final de lagunas luego de aplicación de filtro de expansión]{\includegraphics[width=0.49\linewidth]{imagenes/lastExpansionZoom}}
	\caption{Efecto de aplicar filtro de expansión a máscara binaria.} 
...
\begin{table}[H]
 ...
 \caption{Referencias de colores en imágenes de agua, resultados de la combinación de las tres imágenes una misma colocando R\_sim en el canal rojo, H\_sim en el canal verde y IM\_L en el canal azul.}
 \label{tablaColores}
\end{table}
...
\begin{itemize}
	...
	\item El DEM resultante resuelve lo siguiente con respecto a HydroSHEDS:	
		...
		\item Además es importante recordar que con el Simulador Hidrológico no se ejecutó un evento real de precipitaciones e inundaciones y que no se disponían de datos de suelo, lo que indica que con mejores condiciones de simulaciones las concordancias globales se cree que deberían mejorar.
